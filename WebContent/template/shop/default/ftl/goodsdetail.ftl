...
<html><head><meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
    ...
    <script>
		...
		function tobuy(){
			...
			if(res){
				...
				$.ajax({
					url:"weixinShopCartController.do?judgeUserLogined",
					type:"POST",
					dataType:"JSON",
					success:function(data){
						if(data.success){
							var url = "weixinShopCartController.do?addGoodsToCart";
							$.ajax({
								url:url,
								type:"POST",
								dataType:"JSON",
								data:{"shopSymbol":"shop","buyNum":buyNum,"goodsId":goodsId},
								success:function(data){
									if(data.success){
										window.location.href="weixinShopCartController.do?goCart&shopSymbol=shop";
									}
								}
							});	
						}else{
							alert("对不起，你没有登陆，不能使用购物车！");
						}
					}
				});
  ...
  <body>
 			<div class="m-hd">
				 ...
				 <div>
				     <a href="weixinShopDealController.do?gomyorder&shopSymbol=shop&accountid=${weixinShopContent.accountid?default("")}" class="uc">用户中心</a>
				 </div>
				 <div>
				     <a href="weixinShopCartController.do?goCart&shopSymbol=shop&accountid=${weixinShopContent.accountid?default("")}" class="cart">购物车<i class="cart_com">1</i></a>
				 </div>
            </div>
             <ul class="sub-menu-list">
            	<li>
                    <a href="weixinShopController.do?goPage&page=index&accountid=${weixinShopContent.accountid?default("")}">首页</a>
                </li>
             	<#list weixinShopContent.categoryList as category>
                <li>
                    <a href="weixinShopController.do?goPage&page=goodslist&categoryid=${category.id}&accountid=${weixinShopContent.accountid?default("")}">${category.name}</a>
                </li>
             	 </#list> 
            </ul>
...
<div class="btn_wrap btn_wrap_nocart btn_wrap_fixed layer_show" id="buyAreaBtm" style="display: none; ">
    ...
    <div class="btn_col">
        <a class="btn btn_cart" id="addCart2" href="javascript:togwc();" style="display: none"><i></i></a>
