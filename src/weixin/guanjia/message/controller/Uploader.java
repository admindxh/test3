...
public class Uploader {
	...
	public void upload() throws Exception {
		...
		try {
			...
			FileOutputStream fos = new FileOutputStream(getPhysicalPath(this.url));
		...
		try {
			...
			for (int i = 0; i < b.length; i++) {
				if (b[i] < 0) {
					int tmp131_129 = i;
					byte[] tmp131_127 = b;
					tmp131_127[tmp131_129] = (byte) (tmp131_127[tmp131_129] + 256);
				}
			}
			ro.write(b);
			ro.flush();
	...
	private boolean checkFileType(String fileName) {
		if (null == allowFiles || allowFiles.length == 0) return false;
	...
	private void parseParams() {
		...
		try {
			...
			while (fii.hasNext()) {
				...
				if (item.isFormField()) {
					this.params.put(item.getFieldName(), getParameterValue(item.openStream()));
	...
	public static byte[] getFileOutputStream(InputStream in) {
		try {
